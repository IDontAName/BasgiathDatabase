...
(function() {
  ...
  function renderEvidenceList() {
    ...
    for (const doc of EVIDENCE_DOCS) {
      const clsType = doc.classification.includes('ALPHA') ? 'ev-alpha' : 'ev-bravo';
      html += `<div class="evidence-card" data-id="${doc.id}">
        <span class="ev-class ${clsType}">${esc(doc.classification)}</span>
        <h4>${esc(doc.title)}</h4>
  ...
  function filterCompendium() {
    ...
    if (results.length > 100) {
      html += `<div class="no-results">${results.length - 100} more results. Refine your search.</div>`;
    }
    container.innerHTML = html;
  ...
  function selectCadet(id) {
    ...
    if (logRefs.length > 0) {
      ...
      for (const ref of logRefs.slice(0, 30)) {
      ...
      if (logRefs.length > 30) {
        html += `<div class="no-results">${logRefs.length - 30} more references...</div>`;
      }
      html += '</div>';
    }
  ...
  function renderSearchResults(query, results) {
    ...
    const renderCategory = (title, items, maxShow, clickFn) => {
      ...
      if (items.length > maxShow) {
        h += `<div class="no-results">${items.length - maxShow} more results...</div>`;
      }
      h += '</div>';
      return h;
    };

    html += renderCategory('Ward Movement Logs', results.ward, 25);
    html += renderCategory('Chantara Perimeter Logs', results.chantara, 25);
    html += renderCategory('Dragon Veil Transit', results.veil, 15);
    html += renderCategory('Evidence Files', results.evidence, 10);
    html += renderCategory('Cadet Compendium', results.compendium, 20);
